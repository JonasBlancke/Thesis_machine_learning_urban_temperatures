...
train = pd.read_csv(file_path, usecols=['T_2M_COR','T_2M', 'T_SK','CBH' ,'T_TARGET', 'city', 'hour','ruralurbanmask', 'COAST', 'POP', 'time'])


train = train.rename(columns={'T_2M_COR': 'T2M', 'T_SK': 'SKT', 'T_2M':'T2M_NC'})
...
temp_train=train[['T2M', 'T_TARGET', 'ruralurbanmask', 'city', 'time', 'T2M_NC']]
...
temp_difference_per_city = temp_train.groupby(['city', 'time']).apply(lambda x: x['T2M'].mean() - x['T2M_NC'].mean())
temp_difference_per_city=temp_difference_per_city.groupby(['city']).mean()
temp_difference_per_city.to_csv('general_lapseratedifference_per_city_train.csv')
