...
# Read CSV files
train_cities = pd.read_csv('/content/drive/MyDrive/thesis/cluster/cities_train_7M.csv')
validation_cities = pd.read_csv('/content/drive/MyDrive/thesis/cluster/cities_validation_7M.csv')
test_cities = pd.read_csv('/content/drive/MyDrive/thesis/cluster/cities_test_7M.csv')

# Merge based on 'city' column
all_cities = pd.concat([train_cities, validation_cities, test_cities], ignore_index=True)
all_cities = all_cities.drop_duplicates(subset=['City'])

# Merge based on 'latitude' and 'longitude' columns
df = pd.merge(clustered, all_cities, on=['Latitude', 'Longitude'])

# Generate random shapes for each type of city
np.random.seed(44)
...
# Create a dictionary of shapes for each city type
city_shapes = {
    'T': 's',
    'V': '^',
    'R': 'o'
...
# Assign shapes based on city type
df['shape'] = df['function'].map(lambda x: city_shapes[x])

# Create a dictionary of labels for each city type
city_labels = {
    'T': 'Train',
    'V': 'Validation',
    'R': 'test'
...
# Reset figure and axis
fig, ax = plt.subplots(figsize=(10, 7))

# Plotting
# Plot all cities with appropriate markers
for _, city in df.iterrows():
    # Determine marker shape based on city type
    marker_shape = city_shapes[city['function']]
    # Plot the city with its appropriate shape
    ax.scatter(X_transformed[city.name, 0], X_transformed[city.name, 1], marker=marker_shape, color=cluster_colors[city['Cluster_x']], edgecolor='black', s=70)
    # Plot 'x' marker for special cities
    if city['City'] in ['Frankfurt', 'Gdansk', 'Marseille']:
        ax.scatter(X_transformed[city.name, 0], X_transformed[city.name, 1], marker='o', color='black', edgecolor='black', s=70)
...
for i, color in enumerate(cluster_colors):
    if i < 3:  # Only add legend for clusters 1, 2, and 3
        legend_handles.append(plt.Line2D([0], [0], color=color, linewidth=3, label=f'Cluster {i+1}'))
# Add legend for city types with black contours
for city_type, shape in city_shapes.items():
    legend_handles.append(plt.Line2D([0], [0], marker=shape, color='k', fillstyle='none',linestyle='none', markersize=10, label=city_labels[city_type]))

# Add legend for representative city as a black circle
legend_handles.append(plt.Line2D([0], [0], marker='o', color='k', fillstyle='full',linestyle='none', markersize=5, label='Representative \ntest city'))
