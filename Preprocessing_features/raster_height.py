...
file_path = '/kyukon/data/gent/vo/000/gvo00041/vsc46127/height/GHS_BUILT_H_AGBH_E2018_GLOBE_R2023A_54009_100_V1_0.tif'
ds = rxr.open_rasterio(file_path, parse_coordinates=True, masked=True)


# Function to replace "tas_" with "height" and remove the "_UrbClim_2015_09_v1.0" part
def modify_variable_string(input_string):
    modified_string = input_string.replace('tas_', 'height_')
...
output_directory = '/kyukon/data/gent/vo/000/gvo00041/vsc46127/height'
...
for filename in os.listdir(input_directory):
    if filename.endswith('.nc'):  # Check if the file is a netCDF file
        ...
        height_reproj = subset.rio.reproject('EPSG:4326')
        ...
        height_built=height_reproj.rio.clip_box(minx=min_lon, miny=min_lat, maxx=max_lon, maxy=max_lat)

        height_built_city=height_built.interp_like(city)
        height_built_city.to_netcdf(output_path)
