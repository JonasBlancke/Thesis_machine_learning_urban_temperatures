...
file_path = '/kyukon/data/gent/vo/000/gvo00041/vsc46127/surface/surface_builtup.tif'
ds = rxr.open_rasterio(file_path, parse_coordinates=True, masked=True)


# Function to replace "tas_" with "surface" and remove the "_UrbClim_2015_09_v1.0" part
def modify_variable_string(input_string):
    modified_string = input_string.replace('tas_', 'surface_')
...
output_directory = '/kyukon/data/gent/vo/000/gvo00041/vsc46127/surface'
...
for filename in os.listdir(input_directory):
    if filename.endswith('.nc'):  # Check if the file is a netCDF file
        ...
        surface_reproj = subset.rio.reproject('EPSG:4326')
        ...
        surface_built=surface_reproj.rio.clip_box(minx=min_lon, miny=min_lat, maxx=max_lon, maxy=max_lat)

        surface_built_city=surface_built.interp_like(city)
        surface_built_city.to_netcdf(output_path)
